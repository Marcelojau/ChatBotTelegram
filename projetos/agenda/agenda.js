...
const { markdown, markup } = require('telegraf/extra')
...
bot.command('semana', async ctx => {
    ...
    ctx.reply('Aqui está sua agenda até na semana que vem'. botoesAgenda(tarefas))
})

bot.command('concluidas', async ctx => {
    const tarefas = await getConcluidas()
    ctx.reply('Aqui estão as suas tarefas concluídas', botoesAgenda(tarefas))
})

bot.command('tarefas', async ctx => {
    const tarefas = await getTarefas()
    ctx.reply('Essa são as tarefas sem data definidas', botoesAgenda(tarefas))
})
// ---- Actions do botoes
...
dataScene.hears(/(Amanha[ãa])/gi, async ctx => {
...
obsScene.enter(ctx => {
    idTarefa = ctx.match[1]
    ctx.reply(`Já pode adicionar suas notações...`)
})
...
obsScene.on('text', async ctx => {
    ...
    await ctx.reply('Observação adicionado!')
    await exibirTarefa(ctx, idTarefa, true)
